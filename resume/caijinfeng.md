...
一、起初在`UED`，负责`css`制作页面

- 参与机票PC端项目重构和维护，机票订单详情，值机选座，低价订阅，往返搜索结果等。
- 参与移动端H5项目重构和维护，我的携程，订单，站内信，收藏等，以及我携响应式改造。

二、后面内部转岗到`基础业务研发部`，负责`javascript`开发，使用 `backbone`+`zepto`+`require` `webpack` `react` `react-native` `crn`等技术栈。

- 主要负责移动端我的收藏、h5首页、h5更多、rn浏览历史等项目开发与维护。
- 负责公共组件开发，如`下拉刷新` `国家码h5/pc` `network状态` `RN组件` 等。
- 参与工作流程改进、推行代码规范、git版本控制、css模块化、打包构建工具的使用、优化与重构。

**厦门灵感方舟网络科技有限公司，前端组长，2013.02至2014.05**
...
**厦门三五互联科技股份有限公司，前端开发，2012.07至2013.01**
